...
async def process_log_event(event: LogEvent, http_client=None, http_client_mod=None, ctx = {}, hostname=''):
  ...
  match event:
    case PlayerChatMessageLogEvent(timestamp, player_name, player_id, message):
      ...
      # --- Legacy / Fallback Logic (if any) ---
      # Kept empty as we expect registry to handle all slash commands from the old file.
      # If there are non-command chats, they just get logged (already done above).
      # --- Legacy commands (WIP migration) ---
      if command_match := re.match(r"/undisplay\s*(?P<category>.*)$", message):
