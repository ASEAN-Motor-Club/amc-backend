...
- Blueprint classes: ``/Game/<pkg>.<Pkg>_C`` — full actors (interaction,
  vendor logic, etc.).
- Raw static meshes: ``/Game/<pkg>.<MeshName>`` — spawned as a
  ``AStaticMeshActor`` with the mesh applied (decor only, no interaction).

Every mesh path here was verified present in the v0.7.19 client PAK via
``mt-pak-extract --search-all``. Spawned objects are session-only; the
